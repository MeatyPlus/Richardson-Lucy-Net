...
data_dir = '/media/sda-4T/liyue/clear_tissue_stitch/clearTissueNew/'
train_model_path='/media/sda-4T/liyue/clear_tissue_stitch/clearTissueNew/train/model_rl/old_clearTissue/'
train_output='/media/sda-4T/liyue/clear_tissue_stitch/clearTissueNew/train/output_rl/'
test_output='/media/sda-4T/liyue/clear_tissue_stitch/clearTissueNew/test/output/'
#data_dir = '/media/sda-4T/liyue/ER/'

#train_model_path='/media/sda-4T/liyue/ER/train/cleartissue_test_model_rl/clearTissue_latest_used/'
#data_dir ='/media/sda-4T/liyue/clear_tissue_stitch/test_widefield/'
#train_model_path='/media/sda-4T/liyue/clear_tissue_stitch/test_widefield/train/model_rl/Nuc/'
#train_output='/media/sda-4T/liyue/clear_tissue_stitch/test_widefield/train/output_rl/'
#test_output='/media/sda-4T/liyue/ER/test/output_rl/stitch/'
#test_output='/media/sda-4T/liyue/clear_tissue_stitch/test_widefield/test/output/Nuc/'
...
normal_pmax=99.9#99.8#98.3#98.3#98.0#99.8#98.0
...
#        print(init.shape)
    ...
    def set_up_unet(self, batch_size):
        ...
        with tf.name_scope('input'):
            ...
            self.a1=tf.Variable(tf.constant(1.0), name='alpha', trainable=True)
            self.b1=tf.Variable(tf.constant(1.0), name='beta', trainable=True)
        # layer 1
        with tf.name_scope('estimation'):
            ...
            #result_prelu_3_1=tf.concat([result_prelu_3,normed_batch_t_down],-1)# not useful
            ...
            #global_ave_pool=tf.reduce_mean(result_prelu_15,axis=[1,2,3],keep_dims=True)
        ...
        with tf.name_scope('update'):
            ...
            self.w[1] = self.gaussian_ker(shape=[3,3,3, num_input_channels,4], name='u_1')#+self.w[3]
            result_conv_1 = tf.nn.conv3d(
                    input=normed_batch, filter=self.w[1],
                    strides=[1, 1, 1, 1, 1], padding='SAME', name='conv_9')#+normed_batch_m
            ...
            result_conv_2_fined=s_sigmoid1(result_conv_2_fine1)#self.batch_norm(x=result_conv_2_fine1, is_training=self.is_traing, name='ub_5'))
            ...
            self.e=temp2#tf.reduce_mean(Estimation,axis=4,keep_dims=True)#result_prelu_2_1
            self.e2=Estimation1#result_prelu_12_1
        ...
        #with tf.name_scope('MSE2'):
            #self.mse2 =self.get_mae(self.e,self.cast_ground_truth)+0.0*self.get_mse(self.e,self.cast_ground_truth)#+self.get_mse(self.input_est,self.cast_image)
...
#            time_start=time.time()
            ...
            for m in range(test_iter_num):
                time_start=time.time()
                test_images,test_GT,label2out,shape=Input.get_data_tiff(data_dir,'test',test_batch_size,normal_pmin,normal_pmax)
