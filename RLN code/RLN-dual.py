...
#       al=tf.Variable(tf.constant(0.03), name='beta', trainable=True)
    ...
    def set_up_unet(self, batch_size):
        ...
        with tf.name_scope('input'):
            ...
            normed_batch_input=self.cast_image1+self.cast_image2


        with tf.name_scope('input'):
            normed_batch=self.cast_image1+self.cast_image2
        ...
        with tf.name_scope('update'):
            ...
            self.w[1] = self.gaussian_ker(shape=[3,3,3, input_data_channel,4], name='u_1')#+self.w[3]
...
#            print(self.prediction.shape)
        ...
        with tf.name_scope('step'):
            ...
            self.learning_rate = tf.train.exponential_decay(0.02,self.global_step,500,0.95,staircase=False) #previous0.025,240,0.9
    ...
    def train(self):
        ...
        #    all_parameters_saver.restore(sess=sess, save_path=checkpoint_path)
            ...
            for k in range(train_iter_num):
                #t1=time.time()
                train_images1,train_images2,train_GT,label2out,shape=Input.get_data_tiff(data_dir,'train',train_batch_size,True)
                #t2=time.time()
                #print(t2-t1)
                mse,ssim,lo,mse2,trainer= sess.run([self.mse,self.SSIM,self.loss,self.mse2,self.train_step],
                           feed_dict={self.shape:shape,self.input_image1:train_images1,self.input_image2:train_images2,self.ground_truth: train_GT,self.is_traing: True})
                #t3=time.time()
                #print(t3-t2)
                sum_los += lo          
                if k % 101 == 0:
...
#            time_start=time.time()
            ...
            for m in range(test_iter_num):
                ...
                w1,image, mse, ssim, los = sess.run([self.w[1],self.prediction, self.mse, self.SSIM, self.loss],
                            feed_dict={self.shape:shape,self.input_image1:test_images1, self.input_image2:test_images2,self.ground_truth: test_GT,self.is_traing:False})
                sum_los += los
                image1=(np.array(image))
                print(image1.shape)
...
#            time_start=time.time()
            ...
            for m in range(test_iter_num):
                ...
                image= sess.run([self.first],feed_dict={self.shape:shape,self.input_image1:test_images1, self.input_image2:test_images2,self.is_traing:False})
                image1=(np.array(image))
