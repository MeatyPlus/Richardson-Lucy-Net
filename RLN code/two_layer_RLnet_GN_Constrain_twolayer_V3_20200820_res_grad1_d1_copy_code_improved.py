...
import argparse
#import tflearn
import makedata3D_V32_alter as Input
...
def global_avg_pool(x):
    b,_,_,_,c=x.shape
    ave=tf.reshape(tf.reduce_mean(x,axis=[1,2,3]),(b,1,c))
    mAx=tf.reshape(tf.reduce_max(x,axis=[1,2,3]),(b,1,c))
    return tf.concat([ave,mAx],axis=-1)
def chan_ave(x):
...
#        print(x_data.shape)
    ...
    def init_w(self, shape, name,stddev=1.0):#0.5和1都可以
        with tf.name_scope('init_w'):
            w = tf.Variable(initial_value=tf.truncated_normal(shape=shape, mean=0.0,stddev=stddev, dtype=tf.float32), name=name)
            return w
    ...
    @staticmethod
    def in_norm(x,is_training, eps=EPS, name='BatchNorm3d'):#GroupNorm(x,G=3,eps=1e-5):
    ...
    def set_up_unet(self, batch_size):
        ...
        with tf.name_scope('update'):
            ...
            result_norm=normed_batch
            self.w[1] = self.gaussian_ker(shape=[3,3,3,1,4], name='u_1')
            result_conv_1 = tf.nn.conv3d(
                    input=result_norm, filter=self.w[1],
            ...
            #EST1=tf.concat([EST,ave_result_prelu_12_u2],-1)
            self.w[2] = self.init_w(shape=[3,3,3,1,8], name='u_3')
            ...
            result_prelu_2_1=Estimation#tf.concat([Estimation,result_prelu_12_1],-1)

            
            Estimation_tile=tf.tile(input=result_prelu_12_1,multiples=[1,1,1,1,4])
            ...
            result_conv_2_fine1_c=tf.concat([result_conv_2_fined,result_prelu_12_1,result_prelu_2_1],-1)#if not ok, try result_prelu_12_1
...
#            print(self.prediction.shape)
        ...
        with tf.name_scope('SSIM2'):
            self.SSIM2=self.get_mse(self.e,self.cast_ground_truth)
        ...
        with tf.name_scope('step'):
            ...
            self.learning_rate = tf.train.exponential_decay(0.02,self.global_step,1000,0.9,staircase=False) #previous0.025,240,0.9
    ...
    def train(self):
        ...
        with tf.Session(config=config) as sess:  # 开始一个会话
            ...
            for k in range(train_iter_num):
                #t1=time.time()
                train_images,train_GT,label2out,shape=Input.get_data_tiff(data_dir,'train',train_batch_size,True)
...
#                    sum_los = 0.0
    ...
    def test(self):
        Input.filenames=[]
        train_dir=model_path #20201105_canbeused/scale4_3/' #save logs files for training process
        checkpoint_path=os.path.join(train_dir,'model.ckpt')
...
#            time_start=time.time()
            ...
            for m in range(test_iter_num):
                time_start=time.time()
                test_images,test_GT,label2out,shape=Input.get_data_tiff(data_dir,'test',test_batch_size)
                ...
                for v in range(test_batch_size):
                    single=reshape_image[v]
                    filenames_out=test_output+'val_'+label2out[v]
                    tiff.imsave(filenames_out,single)
                #########  save output image #####################
                if m % 1 == 0:
                    time_end=time.time()
    ...
    def test1(self):
        Input.filenames=[]
        train_dir=model_path #save logs files for training process
        checkpoint_path=os.path.join(train_dir,'model.ckpt')
        all_parameters_saver = tf.train.Saver()
        with tf.Session() as sess:  # 开始一个会话
#            time_start=time.time()
            ...
            for m in range(test_iter_num):
                time_start=time.time()
                test_images,label2out,shape=InputT.get_data_tiff(data_dir,'test',test_batch_size)
                ...
                for v in range(test_batch_size):
                    single=reshape_image[v]
                    filenames_out=test_output+'single_'+label2out[v]
                    tiff.imsave(filenames_out,single)
                #########  save output image #####################
                if m % 1 == 0:
                    time_end=time.time()
